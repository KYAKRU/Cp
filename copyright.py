...
moye_moye = 900  # 15 minutes in seconds
...
@RiZoeL.on_message(filters.group & filters.command(["stickerautodelete"]))
async def set_sticker_auto_delete_time(_, message):
    try:
        command_parts = message.text.split()
        auto_delete_time = int(command_parts[1])

        global moye_moye
        moye_moye = auto_delete_time

        await message.reply_text(f"Auto-delete time for stickers set to {auto_delete_time} seconds.")
    except (IndexError, ValueError):
        await message.reply_text("Invalid command usage. Correct usage: /stickerautodelete <time_in_seconds>")
